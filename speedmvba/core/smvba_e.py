...
def speedmvba(sid, pid, shard_id, N, f, PK, SK, PK2s, SK2, input, decide, receive, send, predicate=lambda x: True, logger=None):
    ...
    def views():
            ...
            if tag == 'yn':
                ...
                try:
                    for (k, sig_k) in proof:
                        assert ecdsa_vrfy(PK2s[k % N], hash_e, sig_k)
            ...
            if tag == 'no':
                ...
                try:
                    for (k, sig_nono) in proof:
                        assert ecdsa_vrfy(PK2s[k % N], digest_no_no, sig_nono)
                except AssertionError:
        ...
        while not start_wait_for_halt.is_set():
            ...
            for j in range(N):
                ...
                spbc = gevent.spawn(strongprovablebroadcast, sid + 'SPBC' + str(j), pid, shard_id, N, f, PK2s, SK2, j,
                        ...
                        if aba_tag == 'vote':
                            ...
                            if vote_msg[1] == 0:
                                ...
                                try:
                                    # vrify sigmas_no
                                    for (k, sig_k) in vote_msg[3]:
                                        assert ecdsa_vrfy(PK2s[k % N], hash_pre, sig_k)
                                ...
                                try:
                                    # vrify no_no
                                    digest_no_no = hash(str((sid, Leader, r, 'vote')))
                                    assert ecdsa_vrfy(PK2s[sender % N], digest_no_no, vote_msg[4])
                                except AssertionError:
                                    # if logger is not None: logger.info("vote no failed!")
                ...
                if halt_tag == 'halt':
                    ...
                    try:
                        ...
                        for (k, sig_k) in halt_msg[3]:
                            assert ecdsa_vrfy(PK2s[k % N], hash_f, sig_k)
                    except AssertionError:
                        # if logger is not None: logger.info("vote Signature failed!")
                        # print("vote Signature failed!")
                        continue
