...
def strongprovablebroadcast(sid, pid, shard_id, N, f, PK2s, SK2, leader, input, output, receive, send, r, logger=None, predicate=lambda x: True):
    ...
    while True:
        ...
        if msg[0] == 'SPBC_SEND':
            ...
            (_, m) = msg
            if j % N != leader:
                if logger is not None: logger.info("A SPBC_SEND message from node %d other than leader %d" % (j % N, leader))
                print("A SPBC_SEND message from node %d other than leader %d" % (j % N, leader))
                continue
            digest1FromLeader = hash(str((sid, m, "ECHO")))
            # print("Node", pid, "has message", m)
            send(leader + shard_id * N, ('SPBC_ECHO', ecdsa_sign(SK2, digest1FromLeader)))
            ...
            try:
                ...
                assert ecdsa_vrfy(PK2s[j % N], digest1FromLeader, sig1)
            ...
            try:
                ...
                for (k, sig) in sigmas:
                    assert ecdsa_vrfy(PK2s[k % N], hash_e, sig)
            except AssertionError:
            ...
            send(leader + shard_id * N, ('SPBC_FINAL', ecdsa_sign(SK2, digest2)))
            ...
            digest2 = hash(str((sid, m, "FINAL")))
            try:
                assert ecdsa_vrfy(PK2s[j % N], digest2, sig2)
            ...
            try:
                ...
                for (k, sig) in sigmas2:
                    assert ecdsa_vrfy(PK2s[k % N], hash_f, sig)
