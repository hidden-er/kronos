...
def baisedbinaryagreement(sid, pid, shard_id, N, f, coin, input, decide, receive, send, logger=None):
    ...
    def _recv():
        while True:  # not finished[pid]:
            ...
            assert sender % N in range(N)
            if msg[0] == 'EST':
                # BV_Broadcast message
                _, r, v = msg
                assert v in (0, 1)

                est_values[r][v].add(sender % N)
