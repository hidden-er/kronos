...
def validatedagreement(sid, pid, shard_id, N, f, PK, SK, PK1, SK1, PK2s, SK2, input, decide, receive, send, predicate=lambda x: True, logger=None):
    ...
    for j in range(N):
        ...
        cbc = gevent.spawn(consistentbroadcast, sid + 'CBC' + str(j), pid, shard_id, N, f, PK2s, SK2, j,
    ...
    for j in range(N):
        ...
        commit = gevent.spawn(consistentbroadcast, sid + 'COMMIT-CBC' + str(j), pid, shard_id, N, f, PK2s, SK2, j,
    ...
    permutation_coin = shared_coin(sid + 'PERMUTE', pid, shard_id, N, f,
    ...
    while True:
        ...
        while True:
            ...
            if (pi[r] == a) and (ballot_bit == 0 or ballot_bit == 1):
                if ballot_bit == 1:
                    ...
                    if commit_values[sender % N] is not None and commit_values[sender % N][a] == 0:
                        votes[r].add((sender % N, vote))
                        ballot_counter += 1
        ...
        coin = shared_coin(sid + 'COIN' + str(r), pid, shard_id, N, f,
                           PK, SK,
        ...
        # Only leader gets input
        aba = gevent.spawn(baisedbinaryagreement, sid + 'ABA' + str(r), pid, shard_id, N, f, coin,
