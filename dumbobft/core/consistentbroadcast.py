...
def consistentbroadcast(sid, pid, shard_id, N, f, PK2s, SK2, leader, input, receive, send, logger=None):
    ...
    while True:
        ...
        if msg[0] == 'CBC_SEND' and digestFromLeader is None:
            ...
            (_, m) = msg
            if j % N != leader:
                print("Node %d receives a CBC_SEND message from node %d other than leader %d" % (pid, j % N, leader), msg)
                continue
            digestFromLeader = hash((sid, m))
            #print("Node", pid, "has digest:", digestFromLeader, "for leader", leader, "session id", sid, "message", m)
            send(leader + shard_id * N, ('CBC_ECHO', ecdsa_sign(SK2, digestFromLeader)))

        elif msg[0] == 'CBC_ECHO':
            # CBC_READY message
            #print("I receive CBC_ECHO from node %d" % j)
            if pid != leader:
                print("I reject CBC_ECHO from %d as I am not CBC leader:", j % N)
                continue
            (_, sig) = msg
            try:
            ...
            if j % N != leader:
                print("Node %d receives a CBC_FINAL message from node %d other than leader %d" % (pid, j % N, leader), msg)
                continue
            (_, m, sigmas) = msg
            try:
