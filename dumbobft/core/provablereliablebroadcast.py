...
def provablereliablebroadcast(sid, pid, shard_id, N, f,  PK2s, SK2, leader, input, receive, send, logger=None, epoch=None):
    ...
    if pid == leader:
        ...
        for i in range(N):
            ...
            send(i + shard_id * N, ('VAL', roothash, branch, stripes[i]))
    ...
    while True:  # main receive loop
        ...
        if msg[0] == 'VAL' and fromLeader is None:
            ...
            if sender % N != leader:
                print("VAL message from other than leader:", sender % N)
                continue
            try:
            ...
            # Update
            fromLeader = roothash
            # print("node % d," % pid +" VAL message from node % d" % sender + sid)
            broadcast(('ECHO', roothash, branch, stripe))

        elif msg[0] == 'ECHO':
            (_, roothash, branch, stripe) = msg
            # Validation
            if roothash in stripes and stripes[roothash][sender % N] is not None \
               or sender % N in echoSenders:
                print("Redundant ECHO")
                continue
            try:
                assert merkleVerify(N, stripe, roothash, branch, sender % N)
            except AssertionError as e:
                print("Failed to validate ECHO message:", e)
                continue
            # print("node % d," % pid +" ECHO message from node % d" % sender + sid)

            # Update
            stripes[roothash][sender % N] = stripe
            echoSenders.add(sender % N)
            ...
            (_, roothash, sig) = msg
            # Validation
            if sender % N in ready[roothash] or sender % N in readySenders:
                print("Redundant READY")
                continue
            try:
                digest = hash((sid, roothash))
                assert ecdsa_vrfy(PK2s[sender % N], digest, sig)
            except AssertionError:
                print("Signature share failed in PRBC!", (sid, pid, sender % N, msg))
                continue
            # print("node % d," % pid +" READY message from node % d" % sender + sid)

            # Update
            ready[roothash].add(sender % N)
            readySenders.add(sender % N)
            readySigShares[sender % N] = sig
