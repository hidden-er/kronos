...
def provablebroadcast(sid, pid, shard_id, N, f, PK2s, SK2, leader, input, value_output, recv, send, logger=None):
    """provablebroadcast
    ...
    while True:
        ...
        if msg[0] == 'PB_SEND' and digestFromLeader is None:
            ...
            send(leader + shard_id * N, ('PB_ECHO', ecdsa_sign(SK2, digestFromLeader)))
            if pid != leader:
                value_output(v)
                end = time.time()
                if logger != None:
            ...
            if pid != leader:
                print("I reject PB_ECHO from %d as I am not CBC leader:", j % N)
                continue
            (_, sig) = msg
            try:
