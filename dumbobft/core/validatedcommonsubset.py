...
def validatedcommonsubset(sid, pid, shard_id, N, f, PK, SK, PK1, SK1, PK2s, SK2, input, decide, receive, send, predicate=lambda i, v: True, logger=None):
    ...
    vaba = gevent.spawn(validatedagreement, sid + 'VACS-VABA', pid, shard_id, N, f, PK, SK, PK1, SK1, PK2s, SK2,
    ...
    while True:
        j, vj = value_recv.get()
        try:
            assert predicate(j % N, vj)
            valueSenders.add(j % N)
            values[j % N] = vj
